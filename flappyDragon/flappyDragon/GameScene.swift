...
class GameScene: SKScene {
    ...
    func spawnEnemines(){
        ...
        let eneminesDistance = self.player.size.height * 2.5
        
        //criando o inimigo do topo
        let enemyTop = SKSpriteNode(imageNamed: "enemytop\(enemyNumber)")
        let enemyWitdh = enemyTop.size.width
        let enemyHeight = enemyTop.size.height
        
        enemyTop.position = CGPoint(x: size.width + enemyWitdh/2, y: size.height - initialPosition + enemyHeight/2)
        ...
        //criando o inimigo da base
        let enemyBottom = SKSpriteNode(imageNamed: "enemybottom\(enemyNumber)")
        enemyBottom.position = CGPoint(x: size.width + enemyWitdh/2, y: enemyTop.position.y - enemyTop.size.height - eneminesDistance)
        ...
        let laser = SKNode()
        laser.position = CGPoint(x: enemyTop.position.x + enemyWitdh/2, y: enemyTop.position.y - enemyTop.size.height/2 - eneminesDistance/2)
        laser.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: 1, height: eneminesDistance))
        laser.physicsBody?.isDynamic = false
        laser.physicsBody?.categoryBitMask = scoreCategory
        
        
        let distance = size.width + enemyWitdh
    ...
    func addIntro() {
        ...
        intro.zPosition = 3
        intro.position = CGPoint(x: size.width/2, y: size.height/2 - 210)
        addChild(intro)
